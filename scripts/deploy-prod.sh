...
deploy_sistema() {
  ...
  remote "mkdir -p /opt/prospeccao-sistema/backups && node -e \"const{DatabaseSync}=require('node:sqlite');const d=new DatabaseSync('/opt/prospeccao-sistema/prisma/prospeccao.db');d.exec('PRAGMA wal_checkpoint(TRUNCATE)');d.close()\" 2>/dev/null; cp /opt/prospeccao-sistema/prisma/prospeccao.db /opt/prospeccao-sistema/backups/prospeccao-$(date +%Y%m%d%H%M%S).db && ls -t /opt/prospeccao-sistema/backups/*.db | tail -n +31 | xargs -r rm --"
